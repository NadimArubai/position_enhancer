...
class PositionSolverClient(Node):
    ...
    def _spin_node(self):
        """Keep the node spinning in background thread to process callbacks"""
        while rclpy.ok():
    ...
    def wait_for_service_ready(self, timeout_sec=None):
        ...
        start_time = time.time()
        while not self.service_ready and (time.time() - start_time) < timeout_sec:
            time.sleep(0.1)
        return self.service_ready
...
# Example usage with proper spinning
def main():
